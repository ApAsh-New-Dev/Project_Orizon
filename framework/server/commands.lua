...
--command setcrew
ESX.RegisterCommand('setcrew', 'admin', function(xPlayer, args, showError)
	if ESX.DoesCrewExist(args.crew, args.grade) then
		args.playerId.setCrew(args.crew, args.grade)
	else showError('Crew le grade ou les deux sont invalides') end

end, true, {help = 'assigner une Crew à un joueur', validate = true, arguments = {{name = 'playerId', help = 'id joueur', type = 'player'},{name = 'crew', help = 'nom de Crew', type = 'string'},{name = 'grade', help = 'grade de Crew', type = 'number'}}})
