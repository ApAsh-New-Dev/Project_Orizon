function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, crew, loadout, name, coords)
	...
	function self.setJob(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			print(('[Evoria ESX-Framework] [^3ATTENTION^7] .setJob() Ignorée et invalide usage "%s"'):format(self.identifier))
		end
	end


	function self.setCrew(crew, grade)
		grade = tostring(grade)
		local lastCrew = json.decode(json.encode(self.crew))

		if ESX.DoesCrewExist(crew, grade) then
			local crewObject, gradeObject = ESX.Crew[crew], ESX.Crew[crew].grades[grade]

			self.crew.id    = crewObject.id
			self.crew.name  = crewObject.name
			self.crew.label = crewObject.label

			self.crew.grade        = tonumber(grade)
			self.crew.grade_name   = gradeObject.name
			self.crew.grade_label  = gradeObject.label
			self.crew.grade_salary = gradeObject.salary

			if gradeObject.skin_male then
				self.crew.skin_male = json.decode(gradeObject.skin_male)
			else
				self.crew.skin_male = {}
			end

			if gradeObject.skin_female then
				self.crew.skin_female = json.decode(gradeObject.skin_female)
			else
				self.crew.skin_female = {}
			end

			TriggerEvent('esx:setCrew', self.source, self.crew, lastCrew)
			self.triggerEvent('esx:setCrew', self.crew)
			TriggerEvent('esx:savePlayer', self.source)
		else
			print(('[Evoria ESX-Framework] [^3ATTENTION^7] .setCrew() Ignorée et invalide usage "%s"'):format(self.identifier))
			return
		end
	end
