...
Evoria.Serv = function(eventName, ...)
...
ESX.LoadingPrompt = function(loadingText, spinnerType)
...
RegisterNetEvent('esx:drawCustomNotif')
AddEventHandler('esx:drawCustomNotif', function(loadingText, spinnerType)
	ESX.LoadingPrompt(loadingText, spinnerType)
end)

ESX.drawCustomNotif = function(txt)
	SetNotificationTextEntry("STRING")
	AddTextComponentString(txt)
	if string.len(txt) > 99 and AddLongString then AddLongString(txt) end
	Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", 0, 0, "GTA", "Bonjour", 1.0)
    DrawNotification(0, 1)
end

RegisterNetEvent('esx:drawCustomNotif')
AddEventHandler('esx:drawCustomNotif', function(txt)
	ESX.DrawMissionText(txt)
end)

function ESX.TriggerServerCallback(name, cb, ...)
	Core.ServerCallbacks[Core.CurrentRequestId] = cb

	Evoria.Serv('esx:triggerServerCallback', name, Core.CurrentRequestId, ...)
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									if type == 'item_weapon' then
										Evoria.Serv('esx:giveInventoryItem', selectedPlayerId, type, item, nil)
										menu2.close()
										menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity and quantity > 0 and data.current.count >= quantity then
												Evoria.Serv('esx:giveInventoryItem', selectedPlayerId, type, item, quantity)
												menu3.close()
												menu2.close()
												menu1.close()
											else
			...
			elseif data1.current.action == 'remove' then
				if IsPedOnFoot(playerPed) and not IsPedFalling(playerPed) then
					...
					if type == 'item_weapon' then
						...
						Wait(1000)
						Evoria.Serv('esx:removeInventoryItem', type, item)
					else
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
							...
							if quantity and quantity > 0 and data.current.count >= quantity then
								...
								TaskPlayAnim(playerPed, dict, anim, 8.0, 1.0, 1000, 16, 0.0, false, false, false)
								Wait(1000)
								Evoria.Serv('esx:removeInventoryItem', type, item, quantity)
							else
								ESX.ShowNotification(_U('amount_invalid'))
							end
						end, function(data2, menu2)
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity and quantity > 0 then
									if pedAmmo >= quantity then
										Evoria.Serv('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity)
										menu2.close()
										menu1.close()
									else
