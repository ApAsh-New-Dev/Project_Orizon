...
CreateThread(function()
	while not Config.Multichar do
		...
		if NetworkIsPlayerActive(PlayerId()) then
			exports.chat:setAutoSpawn(false)
			DoScreenFadeOut(0)
			Wait(500)
			Evoria.Serv('esx:onPlayerJoined')
			break
		end
	end
end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	Wait(1780)
    --Hud Money style gta au demarage 
	...
	if Config.EnableHud then
		...
		--Crew
		local crewTpl = '<div>{{crew_label}}{{grade2_label}}</div>'
		local grade2Label = ESX.PlayerData.crew.grade_label ~= ESX.PlayerData.crew.label and ESX.PlayerData.crew.grade_label or ''
		if grade2Label ~= '' then grade2Label = ' - '..grade2Label end

		ESX.UI.HUD.RegisterElement('crew', #ESX.PlayerData.accounts, 0, crewTpl, {
			crew_label = ESX.PlayerData.crew.label,
...
AddEventHandler('esx:setMaxWeight', function(newMaxWeight) 
	...
	if IsVIP then
			ESX.PlayerData.maxWeight = newMaxWeight * 2
	else
		ESX.PlayerData.maxWeight = newMaxWeight
	end
end) 
	--ESX.PlayerData.maxWeight = newMaxWeight end)
...
--Crew System 
RegisterNetEvent('esx:setCrew')
AddEventHandler('esx:setCrew', function(crew)
	ESX.SetPlayerData('crew', crew)
	if Config.EnableHud then
		local grade2Label = crew.grade_label ~= crew.label and crew.grade_label or ''
		if grade2Label ~= '' then grade2Label = ' - '..grade2Label end
		ESX.UI.HUD.UpdateElement('crew', {
			orag_label = crew.label,
			grade2_label = grade2Label
		})
	end
end)
							...
							if weapon then
								...
								if weapon.name ~= currentWeapon.name then 
									...
									if ammoCount ~= currentWeapon.Ammo then
										currentWeapon.Ammo = ammoCount
										Evoria.Serv('esx:updateWeaponAmmo', weapon.name, ammoCount)
									end 
								end   
							end
						end    
		...
		while ESX.PlayerLoaded do
			...
			if DoesEntityExist(ESX.PlayerData.ped) then
				...
				if distance > 1 then
					...
					Evoria.Serv('esx:updateCoords', formattedCoords)
				end
			end
			Wait(1500)
		end
					...
					if IsControlJustReleased(0, 38) then
						if IsPedOnFoot(playerPed) then
							if (closestDistance == -1 or closestDistance > 3) then
								...
								Evoria.Serv('esx:onPickup', v.id) -- _TriggerServerEvent
